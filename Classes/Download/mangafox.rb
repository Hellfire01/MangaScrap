class Download_Mangafox
  ...
  def chapter_link(link)
    ...
    @aff.prepare_chapter("downloading chapter #{data[1]}" + MF_volume_string(data[0]) + " of #{@manga_name} ( link #{@links.index(link) + 1} / #{@links.size} )")
