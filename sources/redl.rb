def redl_volume(db, manganame, dw, volume)
  failure = true
  links = dw.get_links()
  ...
  if (failure == true)
    ...
    puts "done"
    HTML.new(db).generate_chapter_index(manganame)
  end
end

def redl_chapter(db, manganame, dw, chapter, volume)
  failure = true
  links = dw.get_links()
  links.each do |link|
    data = data_extractor_MF(link)
...
def redl_manager(db, manganame, volume, chapter, page)
  ...
  if (volume != nil && chapter == nil && page != nil) # volume + page but no chapter
    ...
    redl_chapter(db, manganame, dw, chapter, volume)
  elsif (volume != nil && chapter == nil && page == nil) # volume only
    redl_volume(db, manganame, dw, volume)
  else
...
def redl_arg_extract(db, manganame)
  volume = nil
  chapter = nil
  page = nil
  ARGV.each do |elem|
  ...
  redl_manager(db, manganame, volume, chapter, page)
end
